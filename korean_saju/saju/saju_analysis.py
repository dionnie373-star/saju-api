...
from .ilgan_strength import IlganStrengthAnalyzer
from .ji_ji import JiJi
from .johu import JohuAnalyzer
...
class SajuAnalysis:
    ...
    def yongsin(self) -> YongsinResult:
        """용신 도출 v3 — 정통 파이프라인 (조후 → 억부 → 격국 상신 → 통관·병약)."""
        return YongsinDeriver.derive(
            saju=self.saju,
            gyeokguk=self.jeonggyeok,
            strength=IlganStrengthAnalyzer.analyze(self.saju),
            johu=JohuAnalyzer.analyze(self.saju),
            oegyeok=self.oegyeok,
        )
